({
  ...
  "encode": ({ inputText, inputPassword, outputText }) => {
    outputText.value = CryptoJS.TripleDES.encrypt(inputText.value, inputPassword.value).toString();
  },
  "decode": ({ inputText, outputText }) => {
    outputText.value = CryptoJS.TripleDES.decrypt(inputText.value, inputPassword.value).toString(CryptoJS.enc.Utf8);
  }
})