({
  ...
  "encode": ({ inputText, inputPassword, outputText }) => {
    outputText.value = CryptoJS.RC4Drop.encrypt(inputText.value, inputPassword.value).toString();
  },
  "decode": ({ inputText, outputText }) => {
    outputText.value = CryptoJS.RC4Drop.decrypt(inputText.value, inputPassword.value).toString(CryptoJS.enc.Utf8);
  }
})