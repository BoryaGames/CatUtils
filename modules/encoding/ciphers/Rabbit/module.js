({
  ...
  "encode": ({ inputText, inputPassword, outputText }) => {
    outputText.value = CryptoJS.Rabbit.encrypt(inputText.value, inputPassword.value).toString();
  },
  "decode": ({ inputText, outputText }) => {
    outputText.value = CryptoJS.Rabbit.decrypt(inputText.value, inputPassword.value).toString(CryptoJS.enc.Utf8);
  }
})