({
  ...
  "encode": ({ inputText, inputPassword, outputText }) => {
    outputText.value = CryptoJS.RC4.encrypt(inputText.value, inputPassword.value).toString();
  },
  "decode": ({ inputText, outputText }) => {
    outputText.value = CryptoJS.RC4.decrypt(inputText.value, inputPassword.value).toString(CryptoJS.enc.Utf8);
  }
})