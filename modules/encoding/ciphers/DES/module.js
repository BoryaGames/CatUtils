({
  ...
  "encode": ({ inputText, inputPassword, outputText }) => {
    outputText.value = CryptoJS.DES.encrypt(inputText.value, inputPassword.value).toString();
  },
  "decode": ({ inputText, outputText }) => {
    outputText.value = CryptoJS.DES.decrypt(inputText.value, inputPassword.value).toString(CryptoJS.enc.Utf8);
  }
})