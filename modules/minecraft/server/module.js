({
  ...
  "load": async ({ address, outputData, icon }) => {
    ...
    try {
      var result = await fetch(`https://api.mcsrvstat.us${(document.querySelector(".blockselect.selected").innerText == "Bedrock" ? "/bedrock" : "")}/3/${encodeURIComponent(address.value)}`, {
        "headers": {
    ...
    outputData.innerHTML = `Сервер: <font color="lime">${result.ip}</font>${result.hostname ? ` (<font color="lime">${result.hostname}</font>)` : ""}<br />Статус: ${result.online ? `<font color="lime">Онлайн</font>` : `<font color="red">Оффлайн</font>`}${result.version ? `<br />Версия: <font color="lime">${result.version}</font>` : ""}${result.software ? `<br />Ядро: <font color="lime">${result.software}</font>` : ""}${result.players ? `<br />Игроки: <font color="lime">${result.players.online} / ${result.players.max}</font>` : ""}`;
    if (result.icon) {
